...
pub struct PulseChannel {
    ...
    vol: f32,
    /// Pulse Width range: 0.0 to 1.0. Maps to 50% to 100%
    pw: f32,
    /// Envelope.
    pub env: Envelope,
}
...
impl PulseChannel {
    ...
    /// Expected input range: 0..=255
    /// PW range: 50% to 100%
    pub fn set_pw(&mut self, value: i32) {
        self.pw = (value & 0xff) as f32 / 255.;
        self.env.reset_timer();
    }
...
impl AudioChannel for PulseChannel {
    fn get_next_sample(&mut self) -> f32 {
        ...
        // Pulse Width
        let pw: f32 = match self.env.get_mask_pw() {
            true => env_value / 2. + 0.5,
            false => self.pw,
        };

        // Output function
        match self.cycletimer <= pw {
            true => vol,   // Hi
            false => -vol, // Lo
        }
    }
